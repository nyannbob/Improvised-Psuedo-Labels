...
class ThresholdScheduler:
    ...
    def update(self, max_probs):
        ...
        factor = avg_max_prob * 2 if avg_max_prob * 2 < self.max_threshold else (self.threshold + avg_max_prob + self.max_threshold) / 3
        increment_f = self.increment if factor >= self.threshold else self.increment / 2
...
def generate_pseudo_labels(model, loader, threshold_scheduler):
    ...
    threshold_scheduler.update(all_max_probs)
